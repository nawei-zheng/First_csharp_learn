...
namespace WindowsFormsApp2
{
    ...
    public class FwUpd
    {
        ...
        *************************************************************/
        ...
        //RDB Bootloader 协议定义格式
        private readonly int RDB_BL_CMD_HEAD_DLE = 0x0D0520FE;
        ...
        * 如下是串口相关指令
        *************************************************************/
        private static readonly byte MSG_COM_DATA_LEN = 9;
        ...
        private static int MsgPack(ref Byte[] dstMsg, Byte[] data)
        {
            ...
            pos += data.Length;
            //XOR值
            // Byte[] checkData = new Byte[3 + data.Length];
            // Cmn.memcpy(ref checkData, 0, dstMsg, 3, checkData.Length);
            // FwUpd xorLog = new FwUpd();
            // xorLog.Log(checkData);
            dstMsg[pos++] = Cmn.GetXORTboxBl(dstMsg, 3, 3 + data.Length);    
            //帧尾
            dstMsg[pos++] = 0x0D;
            dstMsg[pos++] = 0x0A;
            return pos;
